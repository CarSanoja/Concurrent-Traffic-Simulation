...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    if (_luz.getCurrentPhase() != TrafficLightPhase::green) {
        _luz.waitForGreen();
    }
    lck.unlock();
}
...
{
    // FP.6a : In Intersection.h, add a private member _luz of type TrafficLight. At this position, start the simulation of _luz.
    _luz.simulate();
